...
class GoogleCalHandler(http.Controller):
    ...
    def list(self, **kw):
        return http.request.render('calendar_google_handler.listing', {
            ...
            'objects': http.request.env['calendar_google_handler'].search([]),
        })

    @http.route('/calendar_google_handler/calendar_google_handler/objects/<model("calendar_google_handler"):obj>/', auth='public')
